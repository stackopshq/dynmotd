...
echo '  + creating config file for excluded users'
if [[ ! -f /etc/dynmotd.conf ]]; then
  cat > /etc/dynmotd.conf << 'EOF'
...
elif [[ -f '/etc/redhat-release' ]] && command -v tuned-adm > /dev/null
then 
...
elif [[ $(curl --max-time 1 --write-out %{http_code} -s --output /dev/null http://169.254.169.254/openstack/latest/meta_data.json) == '200' ]]
then
  cp ./01_openstack.sh ${DYNMOTD_CUSTOM_SCRIPTS_PATH}/.
fi
